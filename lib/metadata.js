...
function buildMetadata (model) {
  ...
  for (var typeKey in model.entityTypes) {
    ...
    for (var propKey in model.entityTypes[typeKey]) {
      ...
      var tempProperty = {'@Name': propKey, '@Type': property.type};
      if(property.key){
        tempProperty['@Nullable'] = false;
      }
      entityType.Property.push(tempProperty)
  ...
  if(complexTypes.length){
    metaModel['edmx:Edmx']['edmx:DataServices']['Schema']['ComplexType'] = complexTypes;
  }
  return builder.create(metaModel).end({pretty: true})
}
